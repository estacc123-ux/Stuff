local ok_init, init_err = pcall(function()
	...
	local function create(className, properties, children)
		...
		if femrkit_registry_add then
			pcall(femrkit_registry_add, inst);
		end
		return inst;
	end
	...
	do
				...
				if self.__OBJECT_EXISTS == true then
					...
					elseif k == "Data" then
						if writecustomasset then
							local ok, assetid = pcall(writecustomasset, tostring(self._id), v);
							if ok and assetid and type(assetid) == "string" then
								self._frame.Image = assetid;
							end
						end
					elseif k == "Url" then
						if http and http.request then
							pcall(function()
								local res = http.request({ Url = v, Method = "Get" });
								local body = type(res) == "table" and res.Body or res;
								if writecustomasset and body and type(body) == "string" then
									local ok, assetid = pcall(writecustomasset, tostring(self._id), body);
									if ok and assetid and type(assetid) == "string" then
										self._frame.Image = assetid;
									end
								end
		...
		do
			...
			function text:_updateTextBounds()
				local props = self._properties;
				local ok, bounds = pcall(_gettextboundsasync, textService, create("GetTextBoundsParams", {
					Text = props.Text,
					Size = props.Size,
		...
		do
			...
			function triangle:__newindex(k, v)
				if self.__OBJECT_EXISTS == true then
					...
					elseif k == "Filled" then
						-- TODO(femrkit): filled triangle in ImageLabel triangle asset
		...
		do
			...
			function quad:__newindex(k, v)
				if self.__OBJECT_EXISTS == true then
					...
					elseif k == "Filled" then
						-- TODO(femrkit): filled quad in ImageLabel
					elseif k == "PointA" then
						self:_updateVertices({
							{ frame._line1, props.PointA, props.PointB },
					...
					elseif k == "Thickness" then
						...
						frame._line2.Size = _udim2fromoffset(frame._line2.AbsoluteSize.X, thickness);
						frame._line3.Size = _udim2fromoffset(frame._line3.AbsoluteSize.X, thickness);
						frame._line4.Size = _udim2fromoffset(frame._line4.AbsoluteSize.X, thickness);
					elseif k == "Transparency" then
						frame._line1.BackgroundTransparency = 1 - v;
						frame._line2.BackgroundTransparency = 1 - v;
						frame._line3.BackgroundTransparency = 1 - v;
	...
	if genv.typeof == nil then
		genv.typeof = typeof;
	end
	genv.Drawing = drawing;
	Drawing = drawing;
	genv.cleardrawcache = drawing.clear;
	cleardrawcache = drawing.clear;

	local is_render = newcclosure(function(x)
		return tostring(x) == "Drawing";
	end);
	genv.isrenderobj = is_render;
	isrenderobj = is_render;

	genv.getrenderproperty = newcclosure(function(x, y)
		_assert(is_render(x), _stringformat("invalid argument #1 to 'getrenderproperty' (Drawing expected, got %s)", _typeof(x)));
		return x[y];
	end);

	genv.setrenderproperty = newcclosure(function(x, y, z)
		_assert(is_render(x), _stringformat("invalid argument #1 to 'setrenderproperty' (Drawing expected, got %s)", _typeof(x)));
		x[y] = z;
	end);
	getrenderproperty = genv.getrenderproperty;
	setrenderproperty = genv.setrenderproperty;
end)

if not ok_init then
	warn("[femrkit-drawing] init failed: " .. tostring(init_err));
end
